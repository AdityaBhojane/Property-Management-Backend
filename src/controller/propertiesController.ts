...
import { createPropertyService, deletePropertyService, getPropertyByIdService, getPropertyService, updatePropertyService } from "../service/propertyService";
...
export const getPropertyByIdController = async (req: Request, res: Response) => {
    try {
        ...
        const response = await getPropertyByIdService(userId)
        res.status(StatusCodes.OK).json(customSuccessResponse('property fetched successfully',response))
    } catch (error) {
        console.log("get property controller error", error);
        res.status(StatusCodes.BAD_REQUEST).json(customErrorResponse('something is wrong with get property controller', error))
